...
class DataSet:
    ...
    def clean_data(self):
        ...
        for item in self.data:
            if int(item[3]) >= self.seq_length and int(
                    item[3]) <= self.max_frames \
    ...
    def frame_generator(self, batch_size, train_test, data_type):
        ...
        print(
            "Creating %s generator with %d samples." % (train_test, len(data)))
    ...
    def get_extracted_sequence(self, data_type, sample):
        ...
        path = os.path.join(self.sequence_path,
                            filename + '-' + str(self.seq_length) +
                            '-' + data_type + '.npy')
        # print("Sequence path - ", path)
